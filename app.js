document.addEventListener("DOMContentLoaded", () => {
    ...
    // México: usa código de país + 10 dígitos, sin +, espacios ni guiones. Ejemplo: 523221234567
...
Hola, quiero realizar este pedido.
        ...
        if (!/^\d{8,15}$/.test(cleanNumber)) {
            showToast("Revisa el número de WhatsApp");
            return;
        }

        const url = `https://wa.me/${cleanNumber}?text=${encodeURIComponent(message)}`;

        // Se crea el enlace durante el clic para evitar bloqueos de popups en Chrome.
